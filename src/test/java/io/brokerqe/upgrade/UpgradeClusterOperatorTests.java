...
public class UpgradeClusterOperatorTests extends AbstractSystemTests {
    ...
    void setupTestEnvironment() {
        LOGGER.info("[{}] [UpgradeTestPlan] {}", testNamespace, testEnvironment.getTestUpgradePlanContent());
    ...
    void microIncrementalTest(ArgumentsAccessor argumentsAccessor) {
        ...
        if (newClusterOperator) {
            ...
            for (Pod brokerUpgradePod : brokerUpgradePods) {
                getClient().waitForPodReload(testNamespace, brokerUpgradePod, brokerUpgradePod.getMetadata().getName(), Constants.DURATION_3_MINUTES);
            }
        }
