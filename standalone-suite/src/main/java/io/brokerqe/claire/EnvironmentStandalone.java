...
public class EnvironmentStandalone extends Environment {
    ...
    protected EnvironmentStandalone() {
        ...
        rapiDastSaFile = getConfigurationValue(Constants.EV_RAPIDAST_SA_FILE, Constants.PROP_RAPIDAST_SA_FILE, null);
    ...
    private String getOSInfo() {
        ...
        try {
            String[] distroCmd = {"/bin/sh", "-lc", "cat /etc/os-release |grep \"^ID=\" |cut -d \"=\" -f2 | sed -e 's/\"//g'"};
            ...
            String[] versionCmd = {"/bin/sh", "-lc", "cat /etc/os-release |grep \"^VERSION_ID=\" |cut -d \"=\" -f2 | sed -e 's/\"//g'"};
            Process versionProcess = Runtime.getRuntime().exec(versionCmd);
            versionProcess.waitFor(Constants.DURATION_5_SECONDS, TimeUnit.MILLISECONDS);
            if (versionProcess.exitValue() != 0) {
                try (BufferedReader buffReader = distroProcess.errorReader()) {
