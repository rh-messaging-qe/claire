...
public class AmqpProtonDotnetClient extends SystemtestClient {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AmqpProtonDotnetClient.class);

    String getCliExecutableBase() {
        return "cli-proton-dotnet";
    }

    public AmqpProtonDotnetClient(DeployableClient deployableClient, String destinationUrl, String destinationPort, String address, String queue, int messageCount) {
    ...
    public AmqpProtonDotnetClient(DeployableClient deployableClient, String destinationUrl, String destinationPort, String address, String queue, int messageCount, String username, String password) {
        String brokerUri = String.format("amqp://%s:%s", destinationUrl, destinationPort);
        String fqqn = String.format("%s::%s", address, queue);
        Map<String, String> senderOptions = new AmqpCliOptionsBuilder()
                .address(fqqn)
    ...
    public AmqpProtonDotnetClient(DeployableClient deployableClient, String brokerUri, String address, String queue, int messageCount, String saslMechanism, String keystore, String keystorePassword, String trustStore, String trustStorePassword) {
    ...
    public AmqpProtonDotnetClient(DeployableClient deployableClient, String brokerUri, Map<String, String> senderOptions, Map<String, String> receiverOptions) {
        this.deployableClient = deployableClient;
        this.brokerUri = brokerUri;
        this.senderOptions = senderOptions;
        this.receiverOptions = receiverOptions;
    ...
    public AmqpProtonDotnetClient(DeployableClient deployableClient, String brokerUri, Map<String, String> senderOptions, Map<String, String> receiverOptions, boolean secured) {
        this.deployableClient = deployableClient;
        this.brokerUri = brokerUri;
        this.senderOptions = senderOptions;
        this.receiverOptions = receiverOptions;
    ...
    private String[] constructClientCommand(String clientType, Map<String, String> clientOptions, Integer timeout) {
        ...
        if (username != null || password != null) {
            ...
            amendedBrokerUri = String.format("%s://%s:%s@%s", uri.getScheme(), username, password, uri.getAuthority());
        } else {
            amendedBrokerUri = brokerUri;
        }

        List<String> list = new ArrayList<>();
        list.add(getCliExecutableBase() + "-" + clientType);
        list.add(String.format("--broker=%s", amendedBrokerUri));
        list.add("--log-msgs=json");
        if (timeout != null) {
            list.add(String.format("--timeout=%d", timeout));
        }
        for (var opt : clientOptions.entrySet()) {
            if (Set.of("conn-username", "conn-password").contains(opt.getKey())) {
                continue;
            }
