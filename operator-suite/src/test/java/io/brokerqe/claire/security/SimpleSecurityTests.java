...
public class SimpleSecurityTests extends AbstractSystemTests {
    ...
    private final String testNamespace = getRandomNamespaceName("simple-sec-tests", 3);
    ...
    void userPropertiesSecretTest() {
        ...
        usersPropertiesFile = getClient().executeCommandInPod(brokerPod, cmd, Constants.DURATION_10_SECONDS);
        ...
        if (testEnvironmentOperator.getArtemisTestVersion().getVersionNumber() >= ArtemisVersion.VERSION_2_28.getVersionNumber()) {
            ...
            assertThat("artemis-users.properties contained unmasked password.", usersPropertiesFile, containsString("alicesecret"));
            assertThat("artemis-users.properties contained unmasked password.", usersPropertiesFile, containsString("bobsecret"));
            assertThat("artemis-users.properties contained unmasked password.", usersPropertiesFile, containsString("password123"));
            assertThat("artemis-users.properties contained unmasked password.", usersPropertiesFile, containsString("Pass!$%/r/n//"));
        }
